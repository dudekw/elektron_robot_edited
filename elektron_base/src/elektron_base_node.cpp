...
// ==
// =====  zakładamy, że zmienna 'NFComBuf.ReadDrivesPosition.data' zwraca zbiorczą liczbę impulsów enkoderów
// ==
...
// number of encoder ticks per single wheel rotation || == mnożenie przez 4 w sterowniku
...
int timeoutCount = 0;
int left = 0;
double d = 0.314; 		// rozstaw kol w metrach
...
//==== Zmienne obsługujące równania odometrii
double x=0, y=0, D,th=0,x_old=0, y_old=0, th_old=0, xvel=0, thvel=0;
double delta_l=0;
double delta_r=0;
		...
		//{
			...
			if(NF_Interpreter(&NFComBuf, rxBuf, &rxCnt, rxCommandArray, &rxCommandCnt) > 0)
			{
				if(NFComBuf.ReadDrivesPosition.updated){
					ROS_INFO("Wheel pose: %d, %d", NFComBuf.ReadDrivesPosition.data[0], NFComBuf.ReadDrivesPosition.data[1]);

				l_pos_total = NFComBuf.ReadDrivesPosition.data[0]; 

				r_pos_total = NFComBuf.ReadDrivesPosition.data[1];
...
*/
			...
			// == == = == Wyliczenie roznicy przebutych drog dla poszczegolnych kol
			...
			// === === == = Jesli nastąpi nieoczekiwany skok wartosci ktoregokolwiek licznika enkodera => nadpisz poprzednia wartosc nowa wartoscia.	

			if (l_pos_total_now>l_pos_total_old+1 || l_pos_total_now<l_pos_total_old-1||r_pos_total_now>r_pos_total_old+1 || r_pos_total_now<r_pos_total_old-1||i)
			{
			...
			th= th_old-((delta_l-delta_r)/d); // całkowity kąt obrotu
			...
			if(secs_now-secs_last > 0.001||-0.001 > secs_now-secs_last){
				thvel=(th-th_old)/(secs_now-secs_last);
				xvel=D/(secs_now-secs_last);
			}else{
				thvel=0;
				xvel=0;
